...
export default async function PageSite({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // 🛡️ Ajuste de extração: garante que pegue os dados mesmo se a estrutura do JSON variar
  const conteudo = site.conteudo || {};
  
  const headline = conteudo.headline || "Título do Site";
  const subheadline = conteudo.subheadline || "";
  const guia_completo = conteudo.guia_completo || "";
  const beneficios = Array.isArray(conteudo.beneficios) ? conteudo.beneficios : [];
  const imagem = conteudo.imagem || "https://via.placeholder.com/1080x720?text=Sem+Imagem";
  const whatsapp = conteudo.whatsapp || "";

  return (
    <main className="min-h-screen bg-[#020617] text-slate-200 font-sans">
      <section className="relative w-full h-[70vh] flex items-center justify-center overflow-hidden">
        {/* Imagem agora focada em moda, sem ossos! */}
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          <div className="space-y-6">
            ...
            <div className="text-slate-400 leading-relaxed space-y-4 text-lg">
              {/* ✅ Fix: Verifica se guia_completo existe antes do split */}
              {guia_completo ? (
                guia_completo.split('\n').map((par: string, i: number) => <p key={i}>{par}</p>)
              ) : (
                <p>Conheça todos os detalhes deste produto exclusivo.</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white/5 p-8 rounded-3xl border border-white/10 backdrop-blur-md">
            ...
            <ul className="space-y-4">
              {beneficios.length > 0 ? (
                ...
                <li className="text-slate-500 italic text-sm">Qualidade e estilo garantidos.</li>
              )}
            </ul>
          </div>
        </div>
      ...
      <div className="max-w-2xl mx-auto px-6 py-20 text-center">
        <a
          ...
          target="_blank"
          className={`group block w-full ${whatsapp ? 'bg-emerald-600 hover:bg-emerald-500 shadow-[0_0_50px_rgba(16,185,129,0.2)]' : 'bg-gray-600 cursor-not-allowed'} text-white font-black py-6 rounded-2xl transition-all text-2xl uppercase`}
        >
          {whatsapp ? 'GARANTIR MEU ACESSO AGORA 🚀' : 'WhatsApp não configurado'}
        </a>
